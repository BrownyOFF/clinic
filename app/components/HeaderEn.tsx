    ...
    <header className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-md sticky top-0 z-50 border-b border-slate-100 dark:border-slate-800 transition-colors duration-500">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-20 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8 text-sm font-medium text-slate-600 dark:text-slate-300">
          <Link href="/en/about-us" className="hover:text-blue-600 dark:hover:text-blue-400 transition">About Us</Link>
          <Link href="/en/news" className="hover:text-blue-600 dark:hover:text-blue-400 transition">News</Link>
          ...
          <div className="relative group py-8">
            ...
            <div className="absolute top-[70px] left-0 w-64 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-xl rounded-2xl py-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 translate-y-2 group-hover:translate-y-0">
              <Link href="/en/for-patient/documents" className="block px-5 py-2.5 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 hover:text-blue-600 dark:hover:text-blue-400 transition">Required Documents</Link>
              <Link href="/en/for-patient/rehabilitation" className="block px-5 py-2.5 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 hover:text-blue-600 dark:hover:text-blue-400 transition">Child Rehabilitation</Link>
              <Link href="/en/for-patient/paid-services" className="block px-5 py-2.5 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 hover:text-blue-600 dark:hover:text-blue-400 transition">Paid Services</Link>
            </div>
          </div>

          <Link href="/en/directions" className="hover:text-blue-600 dark:hover:text-blue-400 transition">Directions</Link>
          <Link href="/en/team" className="hover:text-blue-600 dark:hover:text-blue-400 transition">Team</Link>
          <Link href="/en/vacancy" className="hover:text-blue-600 dark:hover:text-blue-400 transition">Vacancies</Link>
        </nav>
        ...
        <div className="flex items-center gap-2 md:gap-4 z-50">
          ...
          <Link href="/en/contacts" className="hidden md:flex bg-blue-600 text-white px-6 py-3 rounded-full text-sm font-semibold hover:bg-blue-700 transition shadow-lg shadow-blue-600/20">
            Contact Us
          </Link>
        ...
        {isMobileMenuOpen && (
          ...
          >
            <nav className="flex flex-col gap-5 text-lg font-medium text-slate-700 dark:text-slate-200">
              <Link href="/en/about-us" onClick={closeMenu}>About Us</Link>
              <Link href="/en/news" onClick={closeMenu} className="hover:text-blue-600 dark:hover:text-blue-400 transition">News</Link>
              ...
              <div className="border-l-2 border-blue-500 pl-4 py-1 flex flex-col gap-4 bg-slate-50 dark:bg-slate-800/50 rounded-r-xl">
                ...
                <Link href="/en/for-patient/documents" onClick={closeMenu} className="text-base text-slate-600 dark:text-slate-300">Required Documents</Link>
                <Link href="/en/for-patient/rehabilitation" onClick={closeMenu} className="text-base text-slate-600 dark:text-slate-300">Child Rehabilitation</Link>
                <Link href="/en/for-patient/paid-services" onClick={closeMenu} className="text-base text-slate-600 dark:text-slate-300">Paid Services</Link>
              </div>

              <Link href="/en/directions" onClick={closeMenu} className="hover:text-blue-600 dark:hover:text-blue-400 transition">Directions</Link>
              <Link href="/en/team" onClick={closeMenu} className="hover:text-blue-600 dark:hover:text-blue-400 transition">Team</Link>
              <Link href="/en/vacancy" onClick={closeMenu} className="hover:text-blue-600 dark:hover:text-blue-400 transition">Vacancies</Link>
            </nav>
