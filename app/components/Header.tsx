...
import { usePathname } from "next/navigation"; // Додано імпорт для відслідковування URL
...
export default function Header() {
  ...
  // КАРТА ВІДПОВІДНОСТЕЙ: укр шлях -> англ шлях
  const getEnglishUrl = (path: string) => {
    // Якщо шлях відсутній, кидаємо на головну англійську
    if (!path) return "/en";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 md:gap-4 z-50">
          ...
          <div className="hidden sm:flex bg-slate-100 dark:bg-slate-800 rounded-full p-1 ml-2">
            <span className="text-xs px-3 py-1.5 rounded-full bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-400 font-bold shadow-sm">УКР</span>
            {/* МАГІЯ ТУТ: href={englishUrl} */}
        ...
        {isMobileMenuOpen && (
          ...
          >
            ...
            <div className="flex bg-slate-100 dark:bg-slate-800 rounded-full p-1 mx-auto mt-2 w-max">
              <span className="text-sm px-6 py-2 rounded-full bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-400 font-bold shadow-sm">УКР</span>
              {/* МАГІЯ ДЛЯ МОБІЛЬНОГО: href={englishUrl} */}
              <Link href={englishUrl} className="text-sm px-6 py-2 rounded-full text-slate-500 hover:text-slate-800 dark:hover:text-white font-bold transition">EN</Link>
            </div>
            
            <Link href="/kontakty" onClick={closeMenu} className="w-full bg-blue-600 text-white px-6 py-4 rounded-xl text-base font-semibold hover:bg-blue-700 transition text-center block mt-2">
              Зв'язатися з нами
            </Link>
          </motion.div>
        )}
